...
# Configura tu API Key de Bing Maps aquí
...
# Función para obtener coordenadas usando la API de Bing Maps
def get_coordinates(address):
    ...
    try:
        response = requests.get(BING_GEOCODE_URL, params=params)
        response.raise_for_status()
        data = response.json()
        if data.get('statusCode') == 200:
            results = data.get('resourceSets', [])[0].get('resources', [])[0]
            if 'point' in results:
                coordinates = results['point']['coordinates']
                return coordinates[0], coordinates[1]  # latitud y longitud
        return None, None
