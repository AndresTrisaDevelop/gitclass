...
def get_xml_structure(file_path):
    """Parse the XML file and extract its structure, including attributes."""
    try:
        ...
        def parse_element(element):
            ...
            for child in element:
                ...
                structure[child.tag] = {
                    "line": line_number,
                    "attributes": child.attrib,  # Include attributes
                    "children": parse_element(child)
                }
            return structure

        return {root.tag: {"line": root.sourceline, "attributes": root.attrib, "children": parse_element(root)}}
    except Exception as e:
        messagebox.showerror("Error", f"Error parsing XML file: {e}")
        return None

def compare_structures(structure1, structure2, path="Root"):
    """Compare two XML structures, including attributes, and return detailed discrepancies."""
    discrepancies = []
    ...
    def compare_dicts(dict1, dict2, path):
        ...
        for key in keys1 & keys2:
            # Compare attributes
            attrs1 = dict1[key].get("attributes", {})
            attrs2 = dict2[key].get("attributes", {})
            for attr_key in set(attrs1.keys()) - set(attrs2.keys()):
                discrepancies.append((f"{path}/{key}", attr_key, f"Attribute '{attr_key}' missing in file 2."))
            for attr_key in set(attrs2.keys()) - set(attrs1.keys()):
                discrepancies.append((f"{path}/{key}", attr_key, f"Attribute '{attr_key}' missing in file 1."))
            """for attr_key in set(attrs1.keys()) & set(attrs2.keys()):
                if attrs1[attr_key] != attrs2[attr_key]:
                    discrepancies.append(
                        (f"{path}/{key}", attr_key, f"Attribute '{attr_key}' values differ: '{attrs1[attr_key]}' vs. '{attrs2[attr_key]}'.")
                    )"""
